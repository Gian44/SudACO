#!/usr/bin/env python3
"""
Run CP-ACS or CP-DCM-ACO tests on 9x9 Sudoku instances.

This script tests either:
- CP-ACS (algorithm 0)
- CP-DCM-ACO (algorithm 2)

On all 9x9 instances, running each instance 100 times (default, configurable via --reps).
...
def _read_completed_instances_from_summary(outfile: Path):
    ...
    try:
        ...
        # If we can't read it, treat as empty and continue.
        return set()
    return completed
        ...
        with open(progress_file, 'r', newline='') as f:
            ...
            for row in reader:
                ...
                try:
                    ...
                    continue
                # Parse success/time/cycles with safe fallbacks
...
def main():
    ...
    ap.add_argument('--instances', default='instances/9x9', help='Instances folder (default: instances/9x9)')
    ap.add_argument('--timeout', type=int, default=5, help='Per-run timeout seconds (default: 5)')
    ...
    outfile = outdir / f'results_9x9_{alg_name}.csv'
    progress_file = outdir / f'progress_9x9_{alg_name}.csv'
    ...
    for idx, fp in enumerate(instance_files, start=1):
        ...
        # Determine which reps have already been completed for this instance
        rep_map = progress.get(fp.name, {})
        done_reps = set(rep_map.keys())

        # Reconstruct partial aggregates from completed reps
        successes = 0
        times = []
        cycles_solved = []
