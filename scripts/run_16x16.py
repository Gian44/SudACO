...
Example usage:
  ...
  python scripts/run_16x16.py --alg 2 --best-config --verbose   # CP-DCM-ACO + best_config.json

Dynamic pool (recommended):
  python scripts/run_16x16.py --alg 2 --best-config --pool-workers 2 --verbose
...
def main():
    ...
    ap.add_argument(
        ...
        help='Load hyperparameters from ablation best_config.json (default path if flag has no value). '
             'Requires --alg 2. Output: best_config_results_16x16_*.csv under --outdir')
    args = ap.parse_args()
    if args.run < 1:
    ...
    outfile = outdir / f'{file_prefix}results_16x16_{alg_name}{run_suffix}.csv'
    progress_file = outdir / f'{file_prefix}progress_16x16_{alg_name}{run_suffix}.csv'
